...
MANUALS_ROOT = os.path.join(settings.BASE_DIR, 'manuals')
...
def index_manuals():
    ...
    for root, subdirs, files in os.walk(MANUALS_ROOT):
        section = os.path.basename(root) if root != MANUALS_ROOT else None  # Top-level folders are the sections.
        relative_path = os.path.relpath(root, MANUALS_ROOT)
        folder_name = relative_path if relative_path != '.' else None #folder name

        for file in files:
            file_path = os.path.join(root, file)
            relative_file_path = os.path.relpath(file_path, settings.BASE_DIR) #relative path to base_dir
